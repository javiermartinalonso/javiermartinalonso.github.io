...
// def serviceId 
   ...
   def startSerialNumber = 21110000
   def startPhoneNumber = 522110000
   def amount = 100
   ...
   for (i=0; i < amount; i++) {
      ...
      def nomePerfilRede =  "${phoneNumber}_perfilDeRedeCDMA"
      def nomePerfilRedeSerial = startSerialNumber + i + 1

      json = new JsonBuilder()

      json {
          orientClass 'PerfilDeRedeCDMA'
          properties { 
            comentario ''
            nome nomePerfilRede
      ...
      json {
          orientClass 'NumeroDeTelefoneRelacionadoComPerfilDeRedeCDMA'
